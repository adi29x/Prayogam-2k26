...
const QRHuntLeaderboard = () => {
  const data = [];

  return (
    <div className="min-h-screen py-12 px-4 md:px-0">
      <div className="text-center mb-16">
        <AnimatedText text="QR Hunt 2k26 " spanText="Leaderboard" spanClass="text-primary" el="h1" className="text-4xl sm:text-5xl md:text-6xl font-black tracking-tighter mb-4 uppercase" />
        <AnimatedSection direction="up" delay={0.2}>
          <p className="text-base sm:text-lg text-gray-500 max-w-2xl mx-auto leading-relaxed font-bold uppercase tracking-widest">
            Real-time rankings based on submission timestamps and code accuracy.
          </p>
        </AnimatedSection>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-border">
                ...
                <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Name / Team</th>
                <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Timestamp</th>
              ...
              {data.length > 0 ? (
                data.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-8 py-6 text-sm font-black text-gray-400 uppercase tracking-[0.15em]">{item.time}</td>
                    <td className="px-8 py-6 text-right font-black text-primary tracking-tight text-lg uppercase uppercase">{item.status}</td>
                  </tr>
                ))
              ) : (
                <tr>
                  <td colSpan="4" className="px-8 py-20 text-center">
                    <p className="text-sm font-black text-gray-300 uppercase tracking-[0.3em]">Game in Progress: No Submissions Verified Yet.</p>
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <div className="max-w-5xl mx-auto mt-12 text-center">
        <p className="text-[10px] text-gray-400 font-bold uppercase tracking-[0.3em]">Rankings are subject to manual audit of DM timestamps and code accuracy.</p>
      </div>
    </div>
  );
};

export default QRHuntLeaderboard;
